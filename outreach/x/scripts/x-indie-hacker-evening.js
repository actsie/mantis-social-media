#!/usr/bin/env node
/**
 * x-indie-hacker-evening.js
 * Evening wave of indie hacker outreach.
 * Run at 8:00 PM PST daily.
 */
...
const MIN_HOUR      = 20;   // 8:00 PM PST
const MAX_HOUR      = 23;   // 11:00 PM PST
const MIN_GAP       = 35;   // Reduced for 3-session evening wave
const CLUSTER_MAX   = 60;   // At least one tight cluster
const LONG_GAP_MIN  = 55;   // At least one longer gap (was 180, impossible for 3hr window)

// Evening wave - indie hacker focused queries with backup queries for retry logic
...
console.log(`\n📅 X indie hacker EVENING wave for ${today} (${SESSION_COUNT} sessions)\n`);
sessions.forEach(s => console.log(`  ${s.n}. ${s.time}  Search: ${s.label} (${s.type})`));
console.log(`\n  Gaps (min): ${gaps.join(', ')}`);

fs.writeFileSync(SCHED_FILE, JSON.stringify({ date: today, sessionCount: SESSION_COUNT, sessions, track: 'indie-hacker-evening' }, null, 2));
console.log(`\n✓ Written: today-schedule.json (evening wave)\n`);

sessions.forEach(s => {
  const msg = [
    `X INDIE HACKER EVENING SESSION ${s.n}/${SESSION_COUNT} for ${today} — post as @stacydonna0x.`,
    ``,
...
console.log('\n✅ X indie hacker evening planner done.\n');
