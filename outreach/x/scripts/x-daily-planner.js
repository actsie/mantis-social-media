...
sessions.forEach(s => {
  ...
  const msg = [
    ...
    ``,
    `8. Log the session using this EXACT method — run as a single exec block:`,
    `   const fs = require('fs');`,
    ...
    ``,
    `9. Mark session done in today-schedule.json:`,
    ...
    ``,
    `10. FOLLOW BUILDER — find 2-3 new people to engage with and follow:`,
    `   a. Go back to the tweet you engaged with in step 4. Scroll down to the reply section.`,
    `   b. Find 2-3 real people who replied (skip brands, verified accounts, anyone with 1000+ followers).`,
    ...
    `      - Comment on 1 post if there is a genuine hook (same tone rules apply — no filler, no hype)`,
    `        Skip the comment if nothing genuine to say — like-only is fine`,
    `      - Click Follow`,
    `   e. Log each follow using this EXACT method — run as a single exec block:`,
    `      const fs = require('fs');`,
    ...
    `   f. Repeat for up to 2-3 candidates. Stop early if you can't find qualified ones — don't force it.`,
    ``,
    `11. Send a brief Telegram message using the message tool (channel="telegram", target="6241290513"): account name + what was done (reply text or "liked only") + how many new follows added (e.g. "followed 2 new: @handle1, @handle2"). ALWAYS include target="6241290513" — do NOT omit it.`,
  ].filter(Boolean).join('\n');
