...
const FOLLOWER_STATE_FILE = path.join(WORKSPACE, 'outreach/instagram/follower-state.json');
...
sessions.forEach(s => {
  ...
  const msg = [
    ...
    `   Navigate to https://www.instagram.com/stacyd0nna/ and capture the follower count from the profile.`,
    `   `,
    `   Log the follower count:`,
    `   const fs = require('fs');`,
    `   const statePath = '/Users/mantisclaw/.openclaw/workspace/outreach/instagram/follower-state.json';`,
    ...
    ``,
    `⚠️ ANTI-TEMPLATE RULE: Before drafting, check engagement-log.json for comments you've already posted today. If you find yourself using similar phrases, structure, or openings — STOP and write differently. Every comment must feel structurally unique.`,
    ``,
    ...
    `   - Am I summarizing their caption back to them? If yes, rewrite — pick ONE thing and react to it.`,
    ...
    `   Navigate back to the post you commented on.`,
    `   Find your comment and check:`,
    `   - How many likes did your comment get?`,
    `   - Did anyone reply to your comment?`,
    `   `,
    `   Log engagement:`,
    `   const fs = require('fs');`,
    ...
    `       likesOnOurComment: LIKES_COUNT, // replace with actual number (0 if none)`,
    `       repliesToOurComment: REPLIES_COUNT, // replace with actual number (0 if none)`,
    ...
    `   const logPath = '/Users/mantisclaw/.openclaw/workspace/outreach/instagram/engagement-log.json';`,
    `   const log = JSON.parse(fs.readFileSync(logPath, 'utf8'));`,
    `   log.sessions.push({ timestamp, account, postUrl, commentUrl, comment, likes, type, hashtag, note, engagement: null });`,
    `   fs.writeFileSync(logPath, JSON.stringify(log, null, 2));`,
    ...
    ``,
    `12. Send a brief Telegram message using the message tool (channel="telegram", target="6241290513"):`,
    `   Format:`,
    `   IG Session ${s.n}/${COMMENT_COUNT} ✅`,
    `   Account: @ACCOUNT_NAME`,
    `   Action: commented OR liked only`,
    `   Engagement: X likes, Y replies on our comment (check after 60-90s)`,
    `   Followed: N (@handle1, @handle2) OR 0 — no qualifying commenters`,
